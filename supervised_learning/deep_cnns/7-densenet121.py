...
def densenet121(growth_rate=32, compression=1.0):
    ...
    norm_1 = tf.keras.layers.BatchNormalization(axis=3)(X)
    act_1 = tf.keras.layers.ReLU()(norm_1)
    nb_filters = growth_rate * 2
    conv_1 = tf.keras.layers.Conv2D(filters=nb_filters,
                                    kernel_size=(7, 7),
                                    strides=(2, 2),
                                    padding="same",
                                    kernel_initializer=init)(act_1)
    pool_1 = tf.keras.layers.MaxPooling2D(pool_size=(3, 3),
                                          strides=(2, 2),
                                          padding="same")(conv_1)
    ...
    trans_4, nb_filters = transition_layer(dense_4, nb_filters, compression)

    pool_2 = tf.keras.layers.AveragePooling2D(pool_size=(7, 7))(trans_4)
    fc_1 = tf.keras.layers.Dense(units=1000,
                                 activation="softmax",
                                 kernel_initializer=init)(pool_2)

    model = tf.keras.models.Model(inputs=X, outputs=fc_1)
    return model
